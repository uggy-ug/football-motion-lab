...
camera.position.set(4.8,3.15,6.8);
const controls=new OrbitControls(camera,canvas);
controls.enableDamping=true;controls.enablePan=false;controls.minDistance=3.2;controls.maxDistance=12;
controls.target.set(.25,1.25,.3);controls.touches.ONE=THREE.TOUCH.ROTATE;controls.touches.TWO=THREE.TOUCH.DOLLY_PAN;
...
const ground=new THREE.Mesh(new THREE.CircleGeometry(7,64),new THREE.MeshStandardMaterial({color:0x173c32,roughness:.95}));
...
const joints={
 pelvis:sphere(.17),chest:sphere(.15),head:sphere(.23),
 leftHip:sphere(.1),leftKnee:sphere(.105),leftAnkle:sphere(.09),
 rightHip:sphere(.1),rightKnee:sphere(.105),rightAnkle:sphere(.09),
 leftShoulder:sphere(.09),leftElbow:sphere(.08),leftHand:sphere(.075),
 rightShoulder:sphere(.09),rightElbow:sphere(.08),rightHand:sphere(.075)
};
const bones={
 spine:segment(.12),neck:segment(.08),shoulders:segment(.07),hips:segment(.08),
 leftThigh:segment(.085),leftShin:segment(.072),rightThigh:segment(.085),rightShin:segment(.072),
 leftUpperArm:segment(.06),leftForearm:segment(.052),rightUpperArm:segment(.06),rightForearm:segment(.052)
};
...
function makePath(color,points){const g=new THREE.BufferGeometry().setFromPoints(points.map(p=>new THREE.Vector3(...p)));const line=new THREE.Line(g,new THREE.LineBasicMaterial({color,transparent:true,opacity:.55}));scene.add(line);return line}
const samples=Array.from({length:70},(_,i)=>trick.sample(trick.duration*i/69));
makePath(0xffc85a,samples.map(s=>s.rightAnkle));
makePath(0x66e1ff,samples.map(s=>s.ball));
makePath(0x9ae6b4,samples.map(s=>s.com));
...
timeline.max=String(trick.duration);
...
function apply(t){
 const s=trick.sample(t);
 Object.entries(joints).forEach(([k,m])=>setPoint(m,s[k]));
 setSegment(bones.spine,s.pelvis,s.chest);setSegment(bones.neck,s.chest,s.head);setSegment(bones.shoulders,s.leftShoulder,s.rightShoulder);setSegment(bones.hips,s.leftHip,s.rightHip);
 setSegment(bones.leftThigh,s.leftHip,s.leftKnee);setSegment(bones.leftShin,s.leftKnee,s.leftAnkle);setSegment(bones.rightThigh,s.rightHip,s.rightKnee);setSegment(bones.rightShin,s.rightKnee,s.rightAnkle);
 setSegment(bones.leftUpperArm,s.leftShoulder,s.leftElbow);setSegment(bones.leftForearm,s.leftElbow,s.leftHand);setSegment(bones.rightUpperArm,s.rightShoulder,s.rightElbow);setSegment(bones.rightForearm,s.rightElbow,s.rightHand);
 setFoot(leftFoot,s.leftAnkle,s.leftToe);setFoot(rightFoot,s.rightAnkle,s.rightToe);
 const rightActive=s.activeLeg==='right';[bones.rightThigh,bones.rightShin,rightFoot].forEach(m=>m.material=rightActive?activeMat:bodyMat);[bones.leftThigh,bones.leftShin,leftFoot].forEach(m=>m.material=rightActive?bodyMat:activeMat);
 ball.position.set(...s.ball);ball.rotation.x=t*4.6;ball.rotation.z=t*2.1;
 ballArrow.position.copy(ball.position);ballArrow.position.y=.36;ballArrow.setLength(.35+Math.max(0,t-1.56)*1.15,.2,.1);
 com.position.set(...s.com);const floor=[s.com[0],.02,s.com[2]];comLine.geometry.setFromPoints([new THREE.Vector3(...s.com),new THREE.Vector3(...floor)]);comLine.computeLineDistances();
 contactRing.position.set(s.ball[0]-.14,.015,s.ball[2]);contactRing.material.opacity=.75*s.contact;contactRing.scale.setScalar(.7+.5*s.contact);
 const lw=Math.round(s.weights.left);leftWeight.textContent=`${lw}%`;rightWeight.textContent=`${100-lw}%`;
 const idx=Math.max(0,currentPhase(t));phaseTitle.textContent=trick.phases[idx].title;phaseCopy.textContent=trick.phases[idx].copy;document.querySelectorAll('.phase-card').forEach((el,i)=>el.classList.toggle('active-card',i===idx));
 timeline.value=String(t);timeEl.textContent=`${t.toFixed(2)}s`;
}
...
const views={front:[0,2.8,7],side:[7,2.8,.3],rear:[0,2.8,-7],top:[.3,8,.31]};
document.querySelectorAll('[data-view]').forEach(b=>b.addEventListener('click',()=>{camera.position.set(...views[b.dataset.view]);controls.target.set(.25,1.25,.3);controls.update()}));
